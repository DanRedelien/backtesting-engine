...
def test_calibration_cli_support_modules_stay_out_of_bootstrap_and_runtime_modules() -> None:
    module_names = (
    ...
    with patch.dict(sys.modules):
        ...
        parsing = importlib.import_module("backtest_engine.interfaces.cli.calibration.parsing")
        rendering = importlib.import_module("backtest_engine.interfaces.cli.calibration.rendering")

        assert callable(parsing.build_parser)
        assert callable(rendering.format_spread_success)
        assert not [
            name
...
def test_calibration_cli_entrypoint_import_stays_out_of_bootstrap_and_runtime_modules() -> None:
    module_names = (
    ...
    with patch.dict(sys.modules):
        ...
        module = importlib.import_module("backtest_engine.interfaces.cli.calibration.__main__")
