"""Strategy package definition for ``sma_pullback``."""
...
def _build_parameters(strategy_spec: PortfolioStrategySpec) -> SmaPullbackParameters:
    _reject_reserved_parameters(strategy_spec)
    return SmaPullbackParameters.model_validate(
        {
...
def _build_config(
    ...
    del strategy_spec
    sma_parameters = cast(SmaPullbackParameters, parameters)
    item = strategy_items[0]
    return {
        "instrument_id": item.instrument_id,
        "bar_type": item.bar_type,
        "strategy_id": sma_parameters.strategy_id,
        "symbol": sma_parameters.symbol,
        "unit_trade_size": sma_parameters.trade_size,
        "slot_multiplier": _slot_multiplier(slot_sizing),
        "fast_sma_window": sma_parameters.fast_sma_window,
        "slow_sma_window": sma_parameters.slow_sma_window,
        "atr_window": sma_parameters.atr_window,
        "atr_sl_mult": sma_parameters.atr_sl_mult,
        "rr_ratio": sma_parameters.rr_ratio,
        "trade_direction": sma_parameters.trade_direction,
    }
...
STRATEGY_DEFINITION = StrategyPackageDefinition(
    ...
    strategy_path="backtest_engine.strategies.sma_pullback.nautilus_strategy:SmaPullbackStrategy",
    config_path="backtest_engine.strategies.sma_pullback.nautilus_strategy:SmaPullbackStrategyConfig",
